...
const getTeamName = (team) => {
  if (!team) return '';
  if (typeof team === 'string') return team;
  return team.teamName || team.name || team._id || '';
};

const determineWinner = (match) => {
  const { scoreA, scoreB, teamA, teamB } = match;
  if (!Number.isFinite(scoreA) || !Number.isFinite(scoreB)) return getTeamName(match.winner);
  if (scoreA > scoreB) return getTeamName(teamA);
  if (scoreB > scoreA) return getTeamName(teamB);
  return 'Draw';
};
...
const AdminUpdateResultsPage = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      await Promise.all(
        matches.map((match) =>
          apiClient.put(`/matches/${match._id || match.id}`, {
            ...match,
            teamA: getTeamId(match.teamA),
            teamB: getTeamId(match.teamB),
            winner: getTeamId(match.winner),
          })
        ),
      );
